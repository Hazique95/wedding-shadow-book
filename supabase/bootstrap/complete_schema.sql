...
-- Stripe Connect and multi-currency additions
...
create policy "users can read own notifications" on public.user_notifications for select to authenticated using (user_id = auth.uid());
...
create index if not exists notification_jobs_due_idx on public.notification_jobs (status, next_attempt_at asc);
...
alter table public.bookings drop constraint if exists bookings_status_check;
alter table public.bookings add constraint bookings_status_check check (status in ('pending', 'paid', 'confirmed', 'payment_failed', 'cancelled'));